$.fn.GridTree = function(options){
	...
	var defaults = {id:'qt_'+radomId,tbodyId:'qt_tbody_'+radomId,width:'100%',headerRowHeight:28,rowHeight:28,nodeNum:0,params:{'type':'ajax_get_datas', '_token': window.params.csrf_token},level:0};
	var opts = $.extend(defaults, options);
	var jqObj = $(this);
	var treeKeyId;
	var createTBHead = function(){
		var head = [],htmp,rowAlign,rowWidth;
		head.push('<table class="l-grid-header-table qt-grid-tree" cellspacing="0" cellpadding="0" width="'+opts.width+'"><tbody id="'+opts.tbodyId+'"><tr height="'+opts.headerRowHeight+'" class="l-grid-hd-row qt-grid-tree-hd">');
	...
	var loadData = function(params){
		var popts = $.extend(opts,{pid:opts.tbodyId,level:0});
		if (params && popts.params) {
			popts.params = $.extend(popts.params, params);
		}
		loadRow(popts);
	}
	var loadRow = function(_opts){
		$("tr[id^='"+opts.pid+"_']").remove();
		Qk.ajaxRequest(_opts.url, _opts.params, 'GET', function (data) {
	      	if (data.status == 'success' && data.message) {
			    ...
			    var lastNodeId = opts.pid;
			     for(var i=0;i<data.length;i++){
					 ...
					 opts.nodeNum++;
					 body = [];
					 body.push('<tr id="'+nodeId+'" height="'+opts.rowHeight+'" class="l-grid-row qt-grid-tree-row j-'+ttmp[treeKeyId]+'" dataid="'+ttmp[treeKeyId]+'" pdataid="'+nodeId+'" lv="'+_opts.level+'">');
					 ...
					 for(var j=0;j<opts.columns.length;j++){
						 ...
						 if(htmp.id){
							 ...
							 prefix+='<div class="l-grid-tree-space l-grid-tree-link l-grid-tree-link-close qt-tree-img" dataid="'+ttmp[treeKeyId]+'" pdataid="'+nodeId+'" lv="'+_opts.level+'"></div>';
					 ...
					 lastNodeId = nodeId;
					 $('#'+nodeId+" .qt-tree-img").click(function(){
	...
	var changeRowColor = function(){
		var even = false;
		$('.qt-grid-tree-row').each(function(){
